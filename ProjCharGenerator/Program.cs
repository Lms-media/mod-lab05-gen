...
namespace generator {
  public class BigramGenerator {
    ...
    public void plot(string statFileName, string outputFileName) {
      ...
      double[] expectedValues = bigrams.Select(x => (double)getWeight(x.Bigram)).ToArray();
  ...
  public class WordGenerator {
    ...
    public void plot(string statFileName, string outputFileName) {
      ...
      double[] expectedValues = words.Select(x => (double)getWeight(x.Word)).ToArray();
      double[] positions = Enumerable.Range(0, labels.Length).Select(x => (double)x * 3).ToArray();
  ...
  class Program {
    static void bigramText() {
      ...
      bigramGenerator.plot(gen1StatFile, plot1File);
    }
    static void wordText() {
      ...
      wordsGenerator.plot(gen2StatFile, plot2File);
    }
